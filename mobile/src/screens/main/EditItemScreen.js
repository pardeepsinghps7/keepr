...
const EditItemScreen = ({ navigation, route }) => {
  ...
  const [state, setState] = useState({
    ...
    saveForLater: itemDetails?.save_for_later || false,
    rating: itemDetails?.rating || 0,
    dataList: [],
    loading: '',
    selectedListId: '',
    selectedItem: {},
  ...
  const init = async () => {
    ...
    try {
      ...
      const list = getStatusList(itemDetails?.lists?.label);
      let index = list.findIndex(item => item?.key === itemDetails?.status);
      ...
      updateState({
        ...
        selectedListLabel: itemDetails?.lists?.label,
